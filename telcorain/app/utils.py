...
from PyQt6.QtWidgets import QCheckBox, QLabel, QMessageBox, QTableWidget
...
class LinksTableFactory:
    ...
    def _get_or_create_widget(
        self, widget_type: str, original_row_count: int, row: int, col: int
    ):
        widget = None
    ...
    def update_table(
        self,
        current_selection: dict[int, int],
        ...
        for link_id in current_selection:
            ...
            tech_label = self._get_or_create_widget(
                "QLabel", original_row_count, row, 3
            )
            tech_label.setText(links[link_id].tech)

            band_label = self._get_or_create_widget(
                "QLabel", original_row_count, row, 4
            )
            band_label.setText("{:.0f}".format(links[link_id].freq_a / 1000))

            length_label = self._get_or_create_widget(
                "QLabel", original_row_count, row, 5
            )
            length_label.setText("{:.2f}".format(links[link_id].distance))

            name_label = self._get_or_create_widget(
                "QLabel", original_row_count, row, 6
            )
            name_label.setText(links[link_id].name)

            channel_1_box = self._get_or_create_widget(
                "QCheckBox", original_row_count, row, 1
            )
            channel_2_box = self._get_or_create_widget(
                "QCheckBox", original_row_count, row, 2
            )
...
class ErrorBox(QMessageBox):
    ...
    def __init__(
        self, box_title: str = "Error", box_error: str = "Unknown error occurred."
    ):
        """
