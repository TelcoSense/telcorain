...
from database.influx_manager import InfluxChecker, InfluxSignals, influx_man
from database.sql_manager import SqlChecker, SqlSignals, sql_man
...
class MainWindow(QMainWindow):
    def __init__(self, init_logger: InitLogHandler):
        ...
        self.selection_table: QTableWidget = cast(
            QTableWidget, self.findChild(QTableWidget, "tableSelection")
        )
        ...
        self.spin_roll_window: QObject = self.findChild(
            QDoubleSpinBox, "spinRollWindow"
        )
        self.spin_wet_dry_sd: QObject = self.findChild(QDoubleSpinBox, "spinWetDrySD")
        self.spin_baseline_samples: QObject = self.findChild(
            QSpinBox, "spinBaselineSamples"
        )
        self.spin_output_step: QObject = self.findChild(QSpinBox, "spinOutputStep")
        self.spin_interpol_res: QObject = self.findChild(
            QDoubleSpinBox, "spinInterResolution"
        )
        ...
        self.radio_output_total: QObject = self.findChild(
            QRadioButton, "radioOutputTotal"
        )
        ...
        self.spin_waa_schleiss_val: QObject = self.findChild(
            QDoubleSpinBox, "spinSchleissWaa"
        )
        self.spin_waa_schleiss_tau: QObject = self.findChild(
            QDoubleSpinBox, "spinSchleissTau"
        )
        ...
        self.correlation_spin: QObject = self.findChild(
            QDoubleSpinBox, "correlationSpin"
        )
        self.correlation_filter_box: QObject = self.findChild(
            QRadioButton, "radioRemove"
        )
        self.compensation_box: QObject = self.findChild(
            QRadioButton, "radioCompensation"
        )
        ...
        self.skip_influx_write_box: QObject = self.findChild(
            QCheckBox, "boxSkipInfluxWrite"
        )
        self.force_write_box: QObject = self.findChild(QCheckBox, "boxForce")
        self.window_pointer_combo: QObject = self.findChild(
            QComboBox, "windowPointerCombo"
        )
        self.radio_cnn: QObject = self.findChild(QRadioButton, "radioCNN")
        self.action_external_filter: QObject = self.findChild(
            QAction, "actionExternalFilterLayer"
        )
        ...
        self.radio_realtime.clicked.connect(
            lambda a: self.box_only_overall.setChecked(False)
        )
        self.radio_realtime.clicked.connect(
            lambda a: self.window_pointer_combo.setCurrentIndex(1)
        )
        self.radio_historic.clicked.connect(
            lambda a: self.write_output_box.setChecked(False)
        )
        ...
        logger.info(
            "%d microwave link's definitions loaded from MariaDB.", len(self.links)
        )
        ...
        self.current_selection = (
            {}
        )  # link channel selection flag: 0=none, 1=A, 2=B, 3=both -> dict: <link_id>: flag
    ...
    def show_overall_results(self, sig_calc_dict: dict):
        ...
        self.results_tabs[rs_id].render_overall_fig(
            sig_calc_dict["start"],
            sig_calc_dict["end"],
            sig_calc_dict["x_grid"],
            sig_calc_dict["y_grid"],
            sig_calc_dict["rain_grid"],
        ...
        if sig_calc_dict["is_it_all"]:
            self.statusBar().showMessage(
                f'Calculation "{self.results_tabs[rs_id].tab_name}" is complete.'
            )
            ...
            self.statusBar().showMessage(
                f'Overall plot in calculation "{self.results_tabs[rs_id].tab_name}" '
                f"is complete. Animation figures are now interpolated..."
            )
            self.results_tabs[rs_id].change_no_anim_notification(
    ...
    def show_animation_results(self, sig_calc_dict: dict):
        ...
        self.results_tabs[rs_id].render_first_animation_fig(
            sig_calc_dict["x_grid"],
            sig_calc_dict["y_grid"],
        ...
        if self.running_realtime is not None:
            ...
            if timediff.total_seconds() < interval:
                ...
                msg = str(
                    f"Realtime iteration #{self.running_realtime.realtime_runs} of calculation "
                ...
                self.realtime_timer.start(
                    int(interval - timediff.total_seconds()) * 1000
                )
            else:
                msg = str(
                    f"Processing realtime calculation iteration #{self.running_realtime.realtime_runs}..."
                )
    ...
    def calculation_error(self, meta_data: dict):
        self.statusBar().showMessage(
            f"Error occurred in calculation \"{self.results_tabs[meta_data['id']].tab_name}\"."
    ...
    def calculation_fired(self):
        ...
        if cp["time_diff"] < 0:  # if timediff is less than 1 hour (in msecs)
            ...
            msg = (
                f"Output frame interval cannot be shorter than initial data resolution."
            )
            logger.warning(msg)
        elif cp["step"] > 59:
        ...
        # elif (
        #     cp["is_cnn_enabled"]
        #     and ((cp["start"].secsTo(cp["end"]) / 60) / cp["step"])
        #     <= CNN_OUTPUT_LEFT_NANS_LENGTH
        # ):
        #     msg = (
        #         f"When using CNN with {cp['step']}m resolution, "
        ...
        #     logger.warning(msg)
            ...
            if cp["is_output_write"]:
                ...
                if cp["is_force_write"]:
                    logger.info(
                        "[DEVMODE] FORCE write activated, all calculations will be ERASED from the database."
                    )
    ...
    def calculation_cancel_fired(self):
        if self.running_realtime is not None:
            ...
            self.statusBar().showMessage(
                f"Realtime calculation has been interrupted after "
    ...
    def new_linkset_fired(self):
        dialog = FormDialog(
            self, "Link Set Creation", "Please, enter a name of the new link set:"
        )
        ...
        if dialog.exec():
            ...
            for link_id in self.current_selection:
                ...
                if self.current_selection[link_id] == 3:
                    ...
                    self.sets_man.modify_link(
                        sel_name, link_id, self.current_selection[link_id]
                    )
    ...
    def copy_linkset_fired(self):
        sel_name = self.lists.currentItem().text()
        dialog = FormDialog(
            self, "Link Set Copy", "Please, enter a name of the new link set copy:"
        )
        ...
        if dialog.exec():
            ...
            self.statusBar().showMessage(
                f'New copy "{new_name}" of link set "{sel_name}" was created.'
            )
    ...
    def choose_path_fired(self):
        self.outputs_path = QFileDialog.getExistingDirectory(
            self, "Select folder for outputs", self.outputs_path
        )
    ...
    def _influx_status_changed(self, status: bool):
        if status:  # True == connected
            ...
            self.status_db_icon_lbl.setPixmap(
                QPixmap("./app/gui/icons/check_green.png")
            )
    ...
    def _sql_status_changed(self, status: bool):
        if status:  # True == connected
            ...
            self.status_sql_icon_lbl.setPixmap(
                QPixmap("./app/gui/icons/check_green.png")
            )
        else:  # False == disconnected
            self.status_sql_state.setText("Disconnected")
    ...
    def _linkset_selected(self, selection: str):
        ...
        try:
            self.link_table_factory.update_table(
                self.current_selection, visible_row_count, self.links
            )
    ...
    def _get_calc_params(self) -> dict:
        ...
        is_temp_compensated = (
            self.compensation_box.isChecked() and self.filter_box.isChecked()
        )
        ...
        is_temp_filtered = (
            self.correlation_filter_box.isChecked() and self.filter_box.isChecked()
        )
        is_output_write = self.write_output_box.isChecked()
        is_history_write = self.write_history_box.isChecked()
        is_force_write = (
            self.force_write_box.isChecked() and self.write_output_box.isChecked()
        )
        is_influx_write_skipped = self.skip_influx_write_box.isChecked()
        is_window_centered = (
            True if self.window_pointer_combo.currentIndex() == 0 else False
        )
        ...
        if is_external_filter_enabled:
            external_filter_params = {
                ...
                "pixel_threshold": int(
                    config_handler.read_option("external_filter", "pixel_threshold")
                ),
                "default_return": (
                    True
                    if config_handler.read_option("external_filter", "default_return")
                    == "True"
                    else False
                ),
                "IMG_X_MIN": float(
                    config_handler.read_option("external_filter", "IMG_X_MIN")
                ),
                "IMG_X_MAX": float(
                    config_handler.read_option("external_filter", "IMG_X_MAX")
                ),
                "IMG_Y_MIN": float(
                    config_handler.read_option("external_filter", "IMG_Y_MIN")
                ),
                "IMG_Y_MAX": float(
                    config_handler.read_option("external_filter", "IMG_Y_MAX")
                ),
            }
        else:
