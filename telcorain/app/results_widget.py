...
from PyQt6.QtCore import QDateTime, QTimer, QTimeZone
...
class ResultsWidget(QWidget):
    def __init__(
        ...
        self.figs_save_info = {
            -1: False
        }  # -1 = overall fig, then 0+ corresponds with animation counter

        # load UI definition from Qt XML file
        uic.loadUi("./app/gui/ResultsWidget.ui", self)

        # lookup for used widgets and define them
        self.overall_plot_layout: QGridLayout = cast(
            QGridLayout, self.findChild(QGridLayout, "layoutOverallPlot")
        )
        self.main_plot_layout: QGridLayout = cast(
            QGridLayout, self.findChild(QGridLayout, "layoutMainPlot")
        )
        self.tab_name_label: QLabel = cast(
            QLabel, self.findChild(QLabel, "labelCalcName")
        )
        self.start_label: QLabel = cast(
            QLabel, self.findChild(QLabel, "labelStartTime")
        )
        self.end_label: QLabel = cast(QLabel, self.findChild(QLabel, "labelEndTime"))
        self.interval_label: QLabel = cast(
            QLabel, self.findChild(QLabel, "labelFrameInterval")
        )
        self.output_label: QLabel = cast(
            QLabel, self.findChild(QLabel, "labelOutputType")
        )
        self.label_no_anim_notify: QLabel = cast(
            QLabel, self.findChild(QLabel, "labelNoAnim")
        )
        self.label_current_fig_time: QLabel = cast(
            QLabel, self.findChild(QLabel, "labelCurrentFig")
        )
        self.slider: QSlider = cast(QSlider, self.findChild(QSlider, "sliderFrames"))
        self.button_play_pause: QPushButton = cast(
            QPushButton, self.findChild(QPushButton, "buttPlayPause")
        )
        self.button_prev: QPushButton = cast(
            QPushButton, self.findChild(QPushButton, "buttPrev")
        )
        self.button_next: QPushButton = cast(
            QPushButton, self.findChild(QPushButton, "buttNext")
        )
        self.button_start: QPushButton = cast(
            QPushButton, self.findChild(QPushButton, "buttStart")
        )
        self.button_end: QPushButton = cast(
            QPushButton, self.findChild(QPushButton, "buttEnd")
        )
        self.butt_save: QPushButton = cast(
            QPushButton, self.findChild(QPushButton, "buttSave")
        )
        self.butt_open: QPushButton = cast(
            QPushButton, self.findChild(QPushButton, "buttOpenFolder")
        )
        self.butt_close: QPushButton = cast(
            QPushButton, self.findChild(QPushButton, "buttClose")
        )
        self.table_params: QTableWidget = cast(
            QTableWidget, self.findChild(QTableWidget, "tableParams")
        )
        ...
        # prepare canvases
        self.overall_canvas = Canvas(
            cp["X_MIN"], cp["X_MAX"], cp["Y_MIN"], cp["Y_MAX"], cp["map_file"], dpi=75
        )
    ...
    def change_no_anim_notification(self, still_interpolating: bool):
        if still_interpolating:
            ...
            self.label_no_anim_notify.setText(
                "Animation figures are being interpolated..."
            )
        else:
    ...
    def render_overall_fig(self, start, end, x_grid, y_grid, rain_grid, calc_data):
        ...
        self._plot_link_lines(
            calc_data, self.overall_canvas.ax, self.overall_link_lines
        )
    ...
    def render_first_animation_fig(self, x_grid, y_grid, rain_grids, calc_data):
        ...
        if self.realtime_writer is not None:
            self.realtime_writer.start_push_results_thread(
                rain_grids, x_grid, y_grid, calc_data
            )
    ...
    def save_fired(self):
        ...
        if not self.figs_save_info[-1]:
            if self.cp["is_pdf"]:
                ...
                dialog.setText(
                    "PDF saves take several seconds in current implementation."
                )
            ...
            overall_file = (
                self.cp["start"].toString("yyyy-MM-dd_HH-mm-ss")
                + "_to_"
    ...
    def _save_figs(self, canvas, file: str, dpi: int):
        if self.cp["is_png"]:
            canvas.print_figure(
                filename=self.figs_full_path + "/" + file + ".png",
                format="png",
                dpi=dpi,
                bbox_inches="tight",
                pad_inches=0.3,
        ...
        if self.cp["is_pdf"]:
            canvas.print_figure(
                filename=self.figs_full_path + "/" + file + ".pdf",
                format="pdf",
                dpi=dpi,
                bbox_inches="tight",
                pad_inches=0.3,
    ...
    def _update_animation_time(self):
        self.current_anim_time = self.real_start_time.addSecs(
            self.cp["output_step"] * self.animation_counter * 60
        )
        self.label_current_fig_time.setText(
            self.current_anim_time.toString("dd.MM.yyyy HH:mm:ss")
        )

    def _update_animation_fig(self):
        self._refresh_fig(
            self.animation_canvas,
            self.animation_x_grid,
            self.animation_y_grid,
            self.animation_grids[self.animation_counter],
            self.anim_annotations,
            is_total=self.cp["is_output_total"],
        )
    ...
    def _refresh_fig(
        self, canvas, x_grid, y_grid, rain_grid, annotations, is_total: bool = False
    ):
        # clear old plots
        ...
        canvas.pc = canvas.ax.pcolormesh(
            x_grid,
            y_grid,
            rain_grid,
        ...
        if is_total:
            canvas.cbar = canvas.fig.colorbar(
                canvas.pc, format="%d", label="Rainfall Total (mm)"
            )
        else:
            canvas.cbar = canvas.fig.colorbar(
                canvas.pc, format="%d", label="Rainfall Intensity (mm/h)"
            )
    ...
    def _plot_link_lines(self, calc_data, ax, link_lines):
        ...
        if self.cp["is_dummy"]:
            new_lines = ax.plot(
                [calc_data.dummy_a_longitude, calc_data.dummy_b_longitude],
            ...
            new_lines = ax.plot(
                [calc_data.site_a_longitude, calc_data.site_b_longitude],
    ...
    def _fill_info(self):
        ...
        table_vals = [
            QLabel(str(self.cp["rolling_hours"])),
            QLabel(str(self.cp["wet_dry_deviation"])),
            QLabel(str(self.cp["baseline_samples"])),
            QLabel(str(self.cp["interpol_res"])),
            QLabel(str(self.cp["idw_power"])),
            QLabel(str(self.cp["idw_near"])),
            QLabel(str(self.cp["idw_dist"])),
            QLabel(str(self.cp["waa_schleiss_val"])),
    ...
    def _set_times(self, start=None, end=None, np_real_start=None):
        if start is not None:
            ...
            self.start_time = QDateTime.fromSecsSinceEpoch(
                int(unix_start), QTimeZone.utc()
            )
        ...
        if np_real_start is not None:
            ...
            self.real_start_time = QDateTime.fromSecsSinceEpoch(
                unix_real_start, QTimeZone.utc()
            )
